...
def generate_valid_multiplicities(sequences, max_multiplicity_edge, values, max_multiplicity_path=None):
    ...
    def backtrack(index, current_multiplicity, element_counts):
        ...
        # need to fix this temporarily just putting this
...
def generate_multiplicity_collections(sequences, values, max_multiplicity, condition_fn, stop_early=False, max_multiplicity_path=None, discard_threshold=None, restart=1, edges=None, counterports=None, conjugacy_type=0):
    ...
    print('what is max mult path here not in if loop', max_multiplicity_path)

    if max_multiplicity_path is None:
        # check is it has been changed here
        print('what is max mult path here in if loop', max_multiplicity_path)
    ...
    if discard_threshold is None:
        ...
        print('check 1.2')
        # print('valid multiplicities generator created', valid_multiplicities)

        # what is valid_multiplicities ? how to see what it contains
        ...
        for multiplicities in valid_multiplicities:
                ...
                # checking condiiton_fn
                print('check 1.2.2.1')
                print('max mult before condition fn:', max_multiplicity)
                
                if condition_fn(collection, values, max_multiplicity[2]):
                    valid_collection = frozenset(collection.items())
                    # Check conjugacy
                    print('check 1.2.3')
                    print('conjugacy type:', conjugacy_type)
                    ...
                    print('Check 1.3 issue with condition_fn')
...
class Topology:
    def __init__(self, filename, N, conjugacy = True, max_multiplicity=None):
        ...
        # self.N = N
        self.N = [1,1,2,2]
        self.max_multiplicity = 2 if max_multiplicity is None else max_multiplicity
    ...
    def generate_solutions(self, stop_early=False, discard_threshold=None, restart=1):
        ...
        # print('values:', values)
        print('check 1')
        print('checking max here', self.max_multiplicity)
...
if __name__ == '__main__':
    ...
    topology = Topology('lattice_square.dat', N=2, conjugacy=False, max_multiplicity=None)
